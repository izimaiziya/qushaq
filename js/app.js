...
(function () {
  ...
  const demoBoundary = document.querySelector('.demo-nav');
  if (!nav) return;
  ...
  function update() {
    ...
    // The interactive app demo has its own nav pill — only suppress the
    // floating header while that pill is actually near the top of the
    // viewport (genuine overlap risk), not permanently after scrolling
    // past it, so the header works normally everywhere else on the page.
    const demoRectTop = demoBoundary ? demoBoundary.getBoundingClientRect().top : null;
    const inDemoDangerZone = demoRectTop !== null && demoRectTop > -60 && demoRectTop < 120;
